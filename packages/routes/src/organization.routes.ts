...
import { z } from "zod";
import { protectedProcedure, resolve } from "./procedure";
...
const list = protectedProcedure.handler(
	async ({ context }): Promise<ResponseType<Organization[]>> => {
		const orgRepo = resolve<OrganizationRepository>(IOrganizationRepository);
...
const get = protectedProcedure
	...
	.handler(async ({ input, context }): Promise<ResponseType<Organization>> => {
		...
		const membership = await orgRepo.findMembership(
...
const deleteOrganization = protectedProcedure
	...
	.handler(async ({ input, context }): Promise<ResponseType<boolean>> => {
		...
		if (!membership || membership.role !== "owner") {
			throw new ORPCError("FORBIDDEN", {
				message: "Only owners can delete the organization",
			});
		}
...
const getMembers = protectedProcedure
	.input(z.object({ organizationId: z.string().uuid() }))
	.handler(
		async ({ input, context }): Promise<ResponseType<OrganizationMember[]>> => {
			const orgRepo = resolve<OrganizationRepository>(IOrganizationRepository);
...
const removeMember = protectedProcedure
	...
	.handler(async ({ input, context }): Promise<ResponseType<boolean>> => {
		...
		const removed = await orgRepo.removeMember(
			input.organizationId,
			input.userId,
		);
